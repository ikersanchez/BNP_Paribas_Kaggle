#Extra Trees script with isotonic calibration.
...
iso_clf = CalibratedClassifierCV(extc,method = "isotonic",cv = 5)
iso_clf.fit(X_train,target)
y_pred = iso_clf.predict_proba(X_test)

print y_pred
pd.DataFrame({"ID": id_test, "PredictedProb": y_pred[:,1]}).to_csv('CalibratedExtratree.csv',index=False)
