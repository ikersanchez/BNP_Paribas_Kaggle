#ExtraTree with distances to KNN 0.45147 Public Leaderboard
...
DistTrain = pd.read_csv("DistTrainknn2.csv")
DistTest = pd.read_csv("DistTestknn2.csv")

#    Distance computation has been performed using R 
#    Full in XGB2 script
...
#    fullmat <- model.matrix(ID ~ .-1,data = full) #Xgb2 script
#    library(FNN)
...
train = pd.concat([train, DistTrain], axis=1)
test = pd.concat([test, DistTest], axis=1)
...
#print y_pred
pd.DataFrame({"ID": id_test, "PredictedProb": y_pred[:,1]}).to_csv('extra_trees_knn.csv',index=False)