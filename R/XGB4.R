#XGB4 + Knn4 Testlogloss = 0.45415, LB = 0.45139
              #xgb4test = 0.45132
...
#Remove variables through VIF
	...
	#v12 v34 v40 not correlated # estudiar estas variables,tenemos plot de clusterizadas
...
#KNN
	...
	dist <-  kn$nn.dist
	colnames(dist) <- c("distKNN1","distKNN2","distKNN3","distKNN4")
...
#Model cv
	...
	min.error <- which.min(cv[,test.logloss.mean])#3355
	set.seed(111)
	model1 <- xgboost(param = param,data = trainmat,label = y,nrounds = min.error) #'3450 test
...
#Submision
	...
	write.csv(sub,"xgb4test.csv",row.names = FALSE)
